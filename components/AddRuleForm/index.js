...
const AddRuleForm = () => {
    ...
    useEffect(() => {
        axiosInstance.get('/athletes/')
            .then(response => {
                setCurrentState((prev) => ({ ...prev, firstAthletesList: response.data, permanentAthletesList: response.data }))
            })
            .catch((error) => {
                console.log(error);
            })
        ...
        axiosInstance.get('/rules/')
            .then(response => {
                setCurrentState((prev)=>({...prev, alreadyExistingRules: response.data }));
            })
            .catch((error) => {
                console.log(error);
            })
    ...
    const onAddAthleteId = () => {
        ...
        setCurrentState((prev) => ({
            ...prev, athletesId: currentState.athletesId.concat(newUserFull)
        }))
    ...
    const onRemoveAthleteId = (e) => {
        ...
        if (index !== -1) {
            ...
            setCurrentState((prev) => ({
            ...prev, athletesId: athletesIDCopy }));
        }
        let f = e.substring(e.indexOf("~ ") + 2);
        axiosInstance.get('/athletes/' + f)
            .then(response => {
                setCurrentState((prev) => ({
                    ...prev, firstAthletesList: currentState.firstAthletesList.concat(response.data)
                }))
            })
            .catch((error) => {
                console.log(error);
    ...
    const onRemoveCondition = (a, b, c, d) => {
        ...
        for (let i = 0; i < conditionsCopy.length; i++) {
            if (conditionsCopy[i].type === a && conditionsCopy[i].op === b && conditionsCopy[i].value1 === c && conditionsCopy[i].value2 === d) {
                ...
                setCurrentState((prev) => ({
            ...prev, conditions: conditionsCopy }));
            }
        }
    }
    ...
    const filterAthletes = () => {
        ...
        for (let p = 0; p < condizioni.length; p++) {
            ...
            axiosInstance.post('/athletes/ask', condition)
                .then(response => {
                    if (currentState.conditions.length === 1 || link === "") {
                        setCurrentState((prev) => ({
                            ...prev, askResult: response.data
                        }));
                        console.log(link)
                        return;
                    } else if (link === "and") {
            ...
            return (
                <div>
                    ...
                    {currentState.askResult.map((currentResult, index) => {
                        return (
                            <div key={index} className=" bg-light p-3 mb-2 d-flex justify-content-between align-center">
    ...
    const onRemoveTemporalCondition = (a, b, c, d) => {
        ...
        for (let i = 0; i < temporalConditionsCopy.length; i++) {
            if (temporalConditionsCopy[i].temporalItem === a && temporalConditionsCopy[i].temporalOperator === b && temporalConditionsCopy[i].temporalValue1 === c && temporalConditionsCopy[i].temporalValue2 === d) {
                ...
                setCurrentState((prev) => ({
                    ...prev, temporalConditions: temporalConditionsCopy
                }));
            }
        }
    }
...
Do you want to automatically set name?`)) {
                ...
                axiosInstance.post('/rules/add/', rule)
    ...
    return (
        currentState!=null && 
        <div>
            <h1 className="mb-4">Create new rule</h1>
